...
int WinMain(int argc, char *argv[])
{
    ...
    while (!input.GetExit())
    {
        ...
        if (input.KeyIsPressed(SDLK_DOWN))
            playerPosition.y += 100 * Game.deltaTime;
        if (input.KeyIsPressed(SDLK_UP))
            playerPosition.y -= 100 * Game.deltaTime;
        if (input.KeyIsPressed(SDLK_LEFT))
            playerPosition.x -= 300 * Game.deltaTime;
        if (input.KeyIsPressed(SDLK_RIGHT))
            playerPosition.x += 300 * Game.deltaTime;
        if (input.KeyDown(SDLK_SPACE))
            Game.camera.ActiveFollow(playerPosition);
        if (input.KeyDown(SDLK_p))
            Game.camera.DeactivateFollow();
        if (input.KeyDown(SDLK_m))
            std::cout << Game.deltaTime;
